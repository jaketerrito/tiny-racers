...
# Sort Population by score from lowest to highest
...
for i,organism in enumerate(population[int(population_size*.5):]):
	new = {}
	for j in range(2): # need to have clever way to randomly select survivors
		new['wgts'] = mate(organism,population[random.randint(i*2,population_size-1)])
		new['wgts'] = mutate(new,1-i/population_size)
